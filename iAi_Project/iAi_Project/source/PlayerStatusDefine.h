...
static const int	PLAYER_MELEE_STRONG_MIN_CHARGE_FRAME = 5;			// 近接攻撃(強)の最短溜め時間					/* 2025.03.27 菊池雅道 攻撃関連の定数追加 */
...
/* プレイヤー処理用定数 */
static const float	PLAYER_HEIGHT							= 160.f;	// 高さ(当たり判定)																									/* 2025.01.09 菊池雅道 移動関連の定数追加 */
static const float	PLAYER_WIDE								= 15.f;		// 幅(当たり判定)																									/* 2025.01.09 菊池雅道 移動関連の定数追加 */
static const float	PLAYER_CLIMBED_HEIGHT					= 20.f;		// 歩きで登れる高さ																									/* 2025.01.09 菊池雅道 移動関連の定数追加 */
static const float	PLAYER_CLIMBED_HEIGHT_STRONG_MELEE		= 50.f;		// 歩きで登れる高さ※近距離攻撃(強)中																				/* 2025.10.19 菊池雅道 移動関連の定数追加 */
static const int	PLAYER_CHARGE_TO_STRONG_TIME			= 30;		// 近接攻撃が強攻撃に切り替わるまでのフレーム数																		/* 2025.01.22 菊池雅道 攻撃関連の定数追加 */
static const int	PLAYER_MOVE_COLLISION_UP				= 0;		// プレイヤーの移動用コリジョン上側																					/* 2025.02.10 菊池雅道 移動関連の定数追加 */
