...
void SceneUi_Crosshairs::Process()
{
	...
	{
		/* プレイヤーが近接攻撃(強)で敵を倒した後の場合 */
		/* ロックオン対象のエネミーを取得 */
		stNearEnemy.pEnemy = this->PlayerStatusList->pGetPlayerLockOnEnemy();

		/* ロックオン対象のエネミーが存在するか確認 */
		if (stNearEnemy.pEnemy != nullptr)
		{
			/* 対象のエネミーの死亡フラグが有効であるか確認 */
			if (stNearEnemy.pEnemy->bGetDeadFlg() == true)
			{
				// 有効である場合
				/* プレイヤー視点でのロックオン状態を"ロックオンされていない"に設定 */
				stNearEnemy.pEnemy->SetPlayerLockOnType(PLAYER_LOCKON_NONE);
	...
	/* 強攻撃ロックオン使用フラグを確認 */
	else if (this->PlayerStatusList->bGetMeleeSearchCollisionUseFlg() == true)
	{
		// 使用する場合
		/* エネミーリストを取得 */
